...
{
  // Google Fontsの設定
  wp_enqueue_style('google-fonts', 'https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@100..900&family=Roboto:ital,wght@0,100;0,300;0,400;0,500;0,700;0,900;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet', array(), null);
  ...
  // ScrollTriggerの読み込み
  wp_enqueue_script('scrolltrigger', 'https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/ScrollTrigger.min.js', array('gsap'), null, true);
...
add_action('pre_get_posts', 'modify_tours_query');
...
function unregister_taxonomy_for_post_type()
{
  ...
  unregister_taxonomy_for_object_type('voice_genre', 'tours');
}

// -----------tours,voiceページの投稿数-----------------
// -----------------------------------------------------
function modify_tours_query($query)
{
  // 管理画面ではなく、メインクエリの場合
  if (!is_admin() && $query->is_main_query()) {
    // 「tours」投稿タイプのアーカイブページの場合
...
function my_dashboard_widget_function()
{
  echo '<ul class="custom_widget">
            ...
            <li>
              <a href="post-new.php?post_type=tours">
                <p>ツアー</p>
...
{ ?>
  <style>
    ...
    body.login h1 a {
      ...
      background-image: url('<?php echo esc_url(get_template_directory_uri() . '/assets/images/common/logo-portfolio_2.png'); ?>')
    }
  </style>
<?php }
